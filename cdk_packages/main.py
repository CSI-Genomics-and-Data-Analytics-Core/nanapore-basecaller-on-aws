...
class Main(cdk.Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.params.image_builder = ImageBuilder(
            self, "ImageBuilder", params=self.params
        )
        self.params.base_ami = BaseAMI(self, "BaseAMI", params=self.params)
        self.params.basecaller_container = BasecallerContainer(
            self, "BasecallerContainer", params=self.params
        )
        self.params.image_build_starter = ImageBuildStarter(
            self, "ImageBuildStarter", params=self.params
        )
        self.params.compute_env_update = ComputeEnvUpdate(
            self, "ComputeEnvUpdate", params=self.params
        )
        self.params.fsx_lustre = FSxLustre(self, "FSXLustre", params=self.params)
        self.params.batch_compute_env = BatchComputeEnv(
            self, "BatchComputeEnv", params=self.params
        )
        self.params.batch_job_queues = BatchJobQueues(
            self, "BatchJobQueues", params=self.params
        )
        self.params.report = Report(self, "Report", params=self.params)
        self.params.spot_interruption_notify = SpotInterruptionNotify(
            self, "SpotInterruptionNotify", params=self.params
        )
