...
class BaseAMI(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, params=None):
        ...
        ecs_agent = imagebuilder.CfnComponent(
            ...
            data=open(
                os.path.join(dirname, "assets", "component_ecs_container_agent.yaml")
            ).read(),
        )
        ...
        cw_agent = imagebuilder.CfnComponent(
            ...
            data=open(
                os.path.join(
                    dirname, "assets", "component_cloudwatch_agent_config.yaml"
                )
            ).read(),
        )
        ...
        fsx_lustre_client = imagebuilder.CfnComponent(
            ...
            version=datetime.datetime.now().strftime("%Y.%m%d.%H%M%S"),
            data=open(
                os.path.join(dirname, "assets", "component_fsx_for_lustre_client.yaml")
            ).read(),
        )
            ...
            block_device_mappings=[
                imagebuilder.CfnImageRecipe.InstanceBlockDeviceMappingProperty(
                    ...
                    ebs=imagebuilder.CfnImageRecipe.EbsInstanceBlockDeviceSpecificationProperty(
                        delete_on_termination=True, volume_size=70, volume_type="gp3"
                    ),
                )
            ],
        )
