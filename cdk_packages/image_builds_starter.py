...
class ImageBuildStarter(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, params=None):
        ...
        self.start_image_build = lambda_.Function(
            self,
            "Start image builds in EC2 Image Builder",
            description="Start image builds in EC2 Image Builder",
            code=lambda_.Code.from_asset(
                os.path.join(dirname, "assets", "lambda_functions", "start_image_build")
            ),
        ...
        self.start_image_build.role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=params.base_ami.pipeline_arns
                + params.basecaller_container.pipeline_arns,
            )
        )
        ...
        ssm.StringParameter(
            self,
            "SSM parameter image build pipelines",
            parameter_name="/ONT-basecaller/image-build-pipelines",
            string_value=json.dumps(
                {
                    "pipelines": params.base_ami.pipeline_arns
                    + params.basecaller_container.pipeline_arns
                }
            ),
        ...
        start_image_builds_rule.add_target(
            targets.LambdaFunction(self.start_image_build)
        )
            ...
            suppressions=[
                {
                    "id": "AwsSolutions-IAM5",
                    "reason": "Resource ARNs reasonably narrowed down.",
                    "appliesTo": params.base_ami.pipeline_arns
                    + params.basecaller_container.pipeline_arns,
                }
            ],
            apply_to_children=True,
        ...
        log_retention_id = [
            obj.node.id
            for obj in cdk.Stack.of(self).node.find_all()
            if re.match("LogRetention[a-z0-9]+$", obj.node.id)
        ][0]
        NagSuppressions.add_resource_suppressions_by_path(
            cdk.Stack.of(self),
