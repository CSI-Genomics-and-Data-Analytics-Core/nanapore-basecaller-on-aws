...
class ImageBuilder(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, params=None):
        ...
        self.ec2_instance_profile = iam.CfnInstanceProfile(
            self,
            "EC2 Image Builder instance_profile",
        ...
        ssm.StringParameter(
            self,
            "SSM parameter CloudWatch agent configuration for AWS Batch container instances",
            parameter_name="/ONT-basecaller/aws-batch-container-instances-cloudwatch-agent-config",
            string_value=open(
                os.path.join(
                    dirname, "assets", "cloudwatch_agent_config_batch_compute.json"
                )
            ).read(),
...
def get_ami_id_from_name(ami_name):
    ...
    data = [
        [image["CreationDate"], image["ImageId"], image["Name"], image["Description"]]
